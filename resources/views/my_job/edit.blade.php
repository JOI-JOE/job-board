      ...
      <form action="{{ route('my-jobs.update', $job) }}" method="POST">
        ...
        <div class="mb-4 grid grid-cols-2 gap-4">
          <div>
            ...
            <x-text-input name="title" form-ref="filters" :value="$job->title"/>
          </div>
          <div>
            <x-label for="location" :required="true">Location</x-label>
            <x-text-input name="location"  form-ref="filters" :value="$job->location" />
          </div>
          <div class="col-span-2">
            <x-label for="salary" :required="true">Salary</x-label>
            <x-text-input name="salary" type="number"  form-ref="filters" :value="$job->salary"/>
          </div>
          <div class="col-span-2">
            <x-label for="description" :required="true">Description</x-label>
            <x-text-input name="description" type="textarea" form-ref="filters" :value="$job->description"/>
          </div>
          <div>
